...
call_methods(MonoObject *obj)
{
	...
	*/
	...
	klass = mono_object_get_class(obj);
	method = mono_class_get_method_from_name(klass, "CreateWrapper", 1);

	string blah = "blah";
	auto myIntPtr = mono_value_box(domain, mono_get_intptr_class(), &blah);
	cout << &blah << "\n";
	args[0] = myIntPtr;
	args[1] = nullptr;


	result = mono_runtime_invoke(method, obj, args, &exception);


	method = mono_class_get_method_from_name(klass, "PrintIntPtr", 1);
	...
	method = mono_class_get_method_from_name(klass, "PrintSexp", 1);
	result = mono_runtime_invoke(method, obj, args, &exception);

	method = mono_class_get_method_from_name(klass, "ForceGarbageCollection", 0);
	result = mono_runtime_invoke(method, obj, args, &exception);
...
create_object(MonoDomain *domain, MonoImage *image)
{
	...
	klass = mono_class_from_name(image, "TestIntPtrInterop", "MyType");
	if (!klass) {
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	file = "F:\\src\\tmp\\TestIntPtrMarshaling\\Debug\\TestIntPtrInterop.exe";
	/*
	...
	*/
	...
	char * blah = "";
	main_function(domain, file, 1, &blah);
